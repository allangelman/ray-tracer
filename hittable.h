...
bool translate::hit(const ray& r, double t_min, double t_max, hit_data& rec) const {
    ray moved_r(r.origin() - offset, r.direction(), r.time());
    if (!ptr->hit(moved_r, t_min, t_max, rec))
        return false;

    rec.hit_point += offset;
    rec.set_face_normal(moved_r, rec.hit_normal);
...
bool rotate_y::hit(const ray& r, double t_min, double t_max, hit_data& rec) const {
    ...
    auto normal = rec.hit_normal;

    p[0] =  cos_theta*rec.hit_point[0] + sin_theta*rec.hit_point[2];
    p[2] = -sin_theta*rec.hit_point[0] + cos_theta*rec.hit_point[2];

    normal[0] =  cos_theta*rec.hit_normal[0] + sin_theta*rec.hit_normal[2];
    normal[2] = -sin_theta*rec.hit_normal[0] + cos_theta*rec.hit_normal[2];
